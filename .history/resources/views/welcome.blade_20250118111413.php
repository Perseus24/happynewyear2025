...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    function goToNext() {
        if (currentIndex === items.length - 1) {
            // If at the last slide, go back to the first slide smoothly
            currentIndex = 0;

            // Remove the transition briefly to reset position, then re-enable the transition
            carouselItems.style.transition = 'none';
            carouselItems.style.transform = `translateX(0%)`;

            // Force reflow to apply the transform
            carouselItems.offsetHeight;

            // Re-enable the transition with a small delay
            setTimeout(() => {
                carouselItems.style.transition = 'transform 0.5s ease-in-out';
                updateCarousel();
            }, 20); // A small delay to ensure smooth looping
        } else {
            currentIndex++;
            updateCarousel();
        }
