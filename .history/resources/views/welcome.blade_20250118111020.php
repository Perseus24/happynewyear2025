...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    function goToNext() {
        // If we are at the last slide, jump to the first slide
        if (currentIndex === items.length - 1) {
            currentIndex = 0;
            // Immediately jump to the first slide (without animation) and apply the transition again
            carouselItems.style.transition = 'none';
            carouselItems.style.transform = `translateX(0%)`;
            
            // Force a reflow to reset the transition (so that it can animate correctly)
            ...
            setTimeout(() => {
                carouselItems.style.transition = 'transform 0.5s ease-in-out';
                updateCarousel();
            }, 20); // Short delay to ensure transition reset
        } else {
            currentIndex++;
            updateCarousel();
        }
