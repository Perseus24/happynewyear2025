...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
  ...
  function goToNext() {
    if (currentIndex === items.length - 1) {
      // When at the last slide, we need to loop back to the first slide
      currentIndex = 0;
      
      // Temporarily disable transition for a smooth reset
      ...
      // Re-enable transition for the smooth effect after a small delay
      setTimeout(() => {
        carouselItems.style.transition = 'transform 0.5s ease-in-out';
        updateCarousel();
      }, 50); // Small delay to ensure the transition is applied properly
    } else {
      currentIndex++;
      updateCarousel();
    }
  ...
  function goToPrev() {
    if (currentIndex === 0) {
      // When at the first slide, go to the last slide
